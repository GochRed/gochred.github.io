...
let prevSnake = INITIAL_SNAKE.map(p => ({...p}));
...
const resetGame = () => {
  ...
  prevSnake = INITIAL_SNAKE.map(p => ({...p}));
...
const drawCompass = (ctx, cx, cy, radius) => {
  ctx.save();
  ctx.translate(cx, cy);
  ...
  const drawPoint = (angle, length, width) => {
    ctx.save();
    ctx.rotate(angle);
